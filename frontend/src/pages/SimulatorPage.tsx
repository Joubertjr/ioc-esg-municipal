...
function getStatusBadgeClasses(status: OdsStatus | null): string {
  if (status === null) return "bg-gray-100 text-gray-500";
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-gray-500">ODS {ods.odsNumber}</span>
        <DeltaBadge delta={ods.delta} />
      </div>
      <p className="text-xs text-gray-700 leading-tight line-clamp-2">{ods.shortName}</p>
...
function EmptyResultsPlaceholder() {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-10 flex flex-col items-center justify-center text-center gap-4">
      ...
      <div>
        <p className="text-sm font-semibold text-gray-700">Nenhuma simulação executada ainda</p>
      ...
      <div className="space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-4">Parâmetros da simulação</h2>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base font-semibold text-gray-900">Distribuição por área (%)</h2>
              <span
              ...
              {INVESTMENT_AREAS.map((area) => (
                <div key={area}>
                  <div className="flex justify-between mb-1">
                    <label htmlFor={`area-${area}`} className="text-sm font-medium text-gray-700">
                      {INVESTMENT_AREA_LABELS[area]}
                    </label>
                    <span className="text-sm text-gray-500 tabular-nums">{allocation[area]}%</span>
                  </div>
                  <input
                    id={`area-${area}`}
                    type="range"
                  ...
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    <span className="text-xs text-gray-400">Impacta:</span>
