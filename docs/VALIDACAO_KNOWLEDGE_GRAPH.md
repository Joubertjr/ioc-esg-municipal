...
*   **Seed de Dados (`seed-knowledge-graph.ts`):** O script popula o banco com as 17 entidades ODS e 48 relacionamentos (24 pares bidirecionais). A inclusão dos metadados de `weight`, `confidence` e `source` (baseado no paper científico *Pradhan 2017*) confere validade acadêmica às conexões (ex: sinergia de +0.78 entre ODS 1 e 3).
...
*   **`graph_service.ts`:** Implementa a função `traverse` utilizando uma **Common Table Expression (CTE) recursiva** no PostgreSQL. Isso permite buscar vizinhos a *N* saltos de distância (multi-hop) em uma única query otimizada, respeitando os filtros temporais (`validUntil IS NULL OR validUntil > NOW()`).
...
2.  **Expansão da Malha:** Incorporar o paper de *Moallemi (2022)* para expandir as 24 interligações iniciais para 50+ arestas, enriquecendo a densidade do grafo.
